...
function setupEventListeners() {
    ...
    document.getElementById('saveProduct').addEventListener('click', async () => {
        const productData = {
            ...
            farmerPrice: parseFloat(document.getElementById('productFarmerPrice').value),
            marketPrice: parseFloat(document.getElementById('productMarketPrice').value),
            stock: parseInt(document.getElementById('productStock').value),
            image: document.getElementById('productImage').value || 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=500',
            description: document.getElementById('productDescription').value
        };

        // Validate
        if (!productData.name || !productData.farmerPrice || !productData.marketPrice || !productData.stock) {
        ...
        if (typeof API !== 'undefined' && API.products) {
            try {
                ...
                if (response.success) {
                    ...
                    if (productsResponse.success) {
                        appData.products = productsResponse.products;
                        renderProducts();
                        renderFarmerDashboard();
                    }
                }
                ...
                document.getElementById('loadingOverlay').classList.remove('active');
                alert('Error adding product: ' + (error.message || 'Please make sure the server is running'));
            }
        } else {
            ...
            const newProduct = {
                ...
                ...productData,
                farmer: appData.currentUser.name,
                farmerId: appData.currentUser.id,
                location: appData.currentUser.farmLocation || 'Unknown',
                availability: productData.stock > 0 ? 'available' : 'out-of-stock'
            };

            appData.products.push(newProduct);
            alert('Product added successfully! (Note: Server not connected, using local storage)');
            document.getElementById('addProductModal').classList.remove('active');
            renderProducts();
            renderFarmerDashboard();
        }
    ...
    // Orders and Settings navigation
    ...
    if (ordersLink) {
        ordersLink.addEventListener('click', (e) => {
            ...
            if (typeof API !== 'undefined' && API.orders) {
                API.orders.getAll().then(response => {
                    if (response.success) {
                        // Render orders here
